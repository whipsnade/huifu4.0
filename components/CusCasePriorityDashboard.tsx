...
import { LayoutGrid, Plus, FileText, Calendar, User, X, CheckCircle2, Clock, Ban, Hash } from 'lucide-react';
import { Process, ProcessStatus } from '../types';
import { cn } from '../lib/utils';

interface CusCasePriorityDashboardProps {
  processes: Process[];
...
export default function CusCasePriorityDashboard({ processes, onOpenDesign, onCreateNew, onUpdateStatus }: CusCasePriorityDashboardProps) {
  ...
  return (
    <div className="flex-1 bg-slate-50 p-8 overflow-y-auto">
      <header className="mb-8 flex justify-between items-end">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 mb-2">案件等级配置列表</h1>
          <p className="text-slate-500 text-sm">管理和查看各客户的案件等级配置及版本状态。</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-20">
        ...
        {activeTab === 'using' && (
          ...
          >
            ...
            <h3 className="font-bold text-slate-900 mb-1 group-hover:text-indigo-600">新建等级配置</h3>
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in duration-300">
            <div className="p-8">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-xl font-bold text-slate-900">新建等级配置</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-1">配置名称</label>
                  <input
                    required
                    type="text"
                    value={formData.processName}
                    onChange={e => setFormData({ ...formData, processName: e.target.value })}
                    placeholder="例如: 标准案件等级配置"
                    className="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:outline-none focus:bg-white transition-all font-medium text-slate-700"
                  />
                </div>
