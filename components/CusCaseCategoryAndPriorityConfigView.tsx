...
interface CusCaseCategoryAndPriorityConfigViewProps {
  initialData?: Process | null;
  onSave: (groups: LogicGroup[]) => void;
}

export default function CusCaseCategoryAndPriorityConfigView({ initialData, onSave }: CusCaseCategoryAndPriorityConfigViewProps) {
  ...
  const customerFields = useMemo(() => {
    ...
    // Only keep 'category' and 'case-level' fields
    return allFields.filter(f => f.id === 'category' || f.id === 'case-level');
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const getOptionWithParents = (opt: Option): string[] => {
      const ids = [opt.id];
      const field = customerFields.find(f => f.id === opt.fieldId);
      if (!field) return ids;
