...
import { LayoutGrid, Plus, MapPin, Calendar, User, X, CheckCircle2, Clock, Ban, Hash } from 'lucide-react';
import { Process, ProcessStatus } from '../types';
import { cn } from '../lib/utils';

interface CusZoneDashboardProps {
  processes: Process[];
...
export default function CusZoneDashboard({ processes, onOpenDesign, onCreateNew, onUpdateStatus }: CusZoneDashboardProps) {
  ...
  return (
    <div className="flex-1 bg-slate-50 p-8 overflow-y-auto">
      <header className="mb-8 flex justify-between items-end">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 mb-2">Zone 设置列表</h1>
          <p className="text-slate-500 text-sm">管理和查看各客户的区域(Zone)配置及版本状态。</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-20">
        ...
        {activeTab === 'using' && (
          ...
          >
            ...
            <h3 className="font-bold text-slate-900 mb-1 group-hover:text-indigo-600">新建 Zone 配置</h3>
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in duration-300">
            <div className="p-8">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-xl font-bold text-slate-900">新建 Zone 配置</h2>
